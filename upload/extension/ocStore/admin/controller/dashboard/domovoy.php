...
class Domovoy extends \Opencart\System\Engine\Controller {
	...
	public function dashboard() {
		...
		foreach ($folders as $key => $folder) {
			...
			if ($cache) {
				$time = $this->date_diff2(date('Y-m-d H:i'), $cache['date']);
	...
	public function calc($dir = false) {
		...
		if (isset($this->request->get['dir']) or $dir) {
			...
			if (!$this->user->hasPermission('modify', 'extension/ocStore/dashboard/domovoy')) {
				...
				$value = $this->model_setting_setting->getValue('domovoy_folders_' . $key);
				if ($value) {
					$this->model_setting_setting->editValue('domovoy', 'domovoy_folders_' . $key, $folder);
				} else {
	...
	public function phpinfo() {
		if ($this->user->hasPermission('modify', 'extension/ocStore/dashboard/domovoy')) {
			...
			$this->response->setOutput($this->load->view('extension/ocStore/dashboard/domovoy_phpinfo', $data));
		}
	}
	...
	private function date_diff2($date1, $date2) {
